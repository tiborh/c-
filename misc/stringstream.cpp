...
const char* infilename = "stringstream.csv";
...
int main() {
  ...
  while( infile.getline( buff,  BUFFSIZE ) && row < ROWS ) {
	...
	while( ss.getline( buff, 10, ',' ) && col < COLS ) {
	  // Next, use the cstdlib atoi function to convert the
	  //  input value that was just read from ss to an int,
	  //  and copy it into the array.
	  // array[row][col] = atoi(buff);
	  // using stringstream for the same is a little more complicated
	  ss1 << buff;
	  ss1 >> array[row][col];
	  //ss1.str("");		// emptying
	  ss1.clear();		// clearing EOF
	  ++col;
	}
