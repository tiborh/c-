...
union_find::union_find(unsigned int s) {
  points = new std::vector<unsigned int>(s);
  init();
}

union_find::union_find(const char* fn) {
...
void union_find::connect(unsigned int a, unsigned int b) {
...
std::ostream& operator<<(std::ostream& os, const union_find& uf) {
  os << "{ ";
