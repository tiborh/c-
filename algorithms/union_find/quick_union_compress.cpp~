...
quick_union_shorter::quick_union_shorter(const char* fn) {
...
std::vector<unsigned int> quick_union_shorter::get_root_and_size(unsigned int a) {
...
void quick_union_shorter::connect(unsigned int a, unsigned int b) {
  if (is_connected(a,b))
    return;
